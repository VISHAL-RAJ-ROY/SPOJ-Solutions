...
class segtree_lazy { // left and right inclusive
    ...
    public:
        segtree_lazy() {};
        segtree_lazy( int N ) { n = N; tree.assign(4*n+4,0); lazy.assign(4*n+4,0); }
        inline int func( int a, int b ) { return a+b; } // any associative function
        inline void upd_lazy( int node, int l, int r, int val ) {  if(l==r) {tree[node] = val;} lazy[node] = val; }
...
int main() {
    ...
    int t; cin>>t; 
    FOR(i,0,t) {
        int q; cin>>q; vii qq(q,{0,0}); vi v; FOR(j,0,q) cin>>qq[j].xx>>qq[j].yy, v.eb(qq[j].xx), v.eb(qq[j].yy);
        gp<int,int> cor_comp = compress_The_Coordinates_1D(v); segtree_lazy tr(sz(v)); int k = 0;
        // error(v);
        FOR(j,0,q) {
            ii qqq = qq[j];
            tr.update(cor_comp[qqq.xx],cor_comp[qqq.yy],++k);
        }
        set<int> fin; 
        FOR(j,0,sz(v)) 
            fin.insert(tr.query(j,j));
        cout<<sz(fin)<<"\n";
    }
    return 0;
}
