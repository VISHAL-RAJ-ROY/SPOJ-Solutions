...
class segtree { // left and right inclusive
    ...
    public:
        segtree() {};
        segtree( int N ) { n = N; tree.assign(4*n+4,0); }
...
bool comp( int a, int b ) {
    return qq[a].yy < qq[b].yy;
}
gp<int,vector<int>> pos;
int main() {
    ios_base::sync_with_stdio(0);
    // read("rr.txt");
    int n; cin>>n; vi v(n,0); FOR(i,0,n) cin>>v[i]; int q; cin>>q; segtree tr(sz(v)); qq.assign(q,{0,0}); vi qqq(q,0), res(q,0);
    FOR(i,0,q) {
        int x, y; cin>>x>>y; qq[i] = {x-1,y-1}; qqq[i] = i;
    } sort(qqq.begin(),qqq.end(),comp); int last = -1;
    FOR(i,0,q) {
        ii c = qq[qqq[i]];
        FOR(j,last+1,c.yy+1) {
            if( !pos[v[j]].empty() ) tr.update(*(pos[v[j]].end()-1),-1);
            pos[v[j]].eb(j); tr.update(j,1);
        } last = c.yy;
        res[qqq[i]] = tr.query(c.xx,c.yy);
    }
