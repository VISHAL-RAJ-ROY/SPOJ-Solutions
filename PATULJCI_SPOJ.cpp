...
class segtree { // left and right inclusive
    private:
        vi v; ve<mm> tree; int n;
    public:
        segtree() {};
        segtree( vi & arr ) { n = sz(arr); SEL(x,arr) v.eb(x); tree.assign(4*n+4,mm()); build(1,0,n-1); }
        inline mm func( mm a, mm b ) {
            if( a.ca == b.ca ) return {a.ca,a.ct+b.ct};
            else if( a.ct > b.ct ) return {a.ca,a.ct-b.ct};
            else return {b.ca,b.ct-a.ct};
        }
        void build( int node, int l, int r ) {
            if( l == r ) tree[node] = {v[l],1};
            else { int m = (l+r)/2, le = (node<<1), ri = le+1; build(le,l,m); build(ri,m+1,r); tree[node] = func(tree[le],tree[ri]);  }
        }
        mm query( int node, int l, int r, int ql, int qr ) {
            if( ql > qr ) return {-1,0};
        ...
        // }
        int query(int l, int r) { return query(1,0,n-1,l,r).ca; } 
        // void update(int index, int inc_by) { update(1,0,n-1,index,inc_by); } 
};
gp<int,vector<int>> pl; 
int main() {
    ios_base::sync_with_stdio(0); 
    // read("rr.txt");
    int N, C; cin>>N>>C; vi v(N,0); FOR(i,0,N) cin>>v[i], pl[v[i]].pb(i); int M; cin>>M; segtree tr(v);
    FOR(i,0,M) {
        int x,y; cin>>x>>y;x--;y--;int can = tr.query(x,y);
        int ct = ub(all(pl[can]),y)-lb(all(pl[can]),x);
        if( ct > (y-x+1)/2 ) cout<<"yes "<<can<<"\n"; else cout<<"no\n";
    }
    return 0;
}
